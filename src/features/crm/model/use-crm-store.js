...
const CRM_COLUMN_ORDER_STORAGE_KEY = "crm:column-order";
const CRM_LEAD_ORDER_STORAGE_KEY = "crm:lead-order";
...
function writeStorageJson(key, value) {
  ...
  try {
    ...
    // Storage cheklovi bo'lsa jim o'tamiz.
  }
}

function loadStoredColumnOrder() {
  const value = readStorageJson(CRM_COLUMN_ORDER_STORAGE_KEY, []);
  return Array.isArray(value) ? value.map(normalizeId).filter(Boolean) : [];
}

function loadStoredLeadOrderByColumn() {
  const value = readStorageJson(CRM_LEAD_ORDER_STORAGE_KEY, {});
  if (!value || Array.isArray(value) || typeof value !== "object") {
    return {};
  }

  return Object.fromEntries(
    Object.entries(value).map(([columnId, ids]) => [
      normalizeId(columnId),
      Array.isArray(ids) ? ids.map(normalizeId).filter(Boolean) : [],
    ]),
  );
}
...
function isArchiveStageName(value) {
  return (
    String(value ?? "")
      .trim()
      .toLowerCase() === SYSTEM_ARCHIVE_STAGE_NAME
  );
}
...
function hasColumnTitle(columns, title, exceptId = null) {
  ...
  return columns.some((column) => {
    ...
    return (
      normalizeColumnTitle(column?.title || column?.name).toLowerCase() ===
      normalizedTitle
    );
  });
}
...
function buildColumnOrder(columns) {
  return columns.map((column) => normalizeId(column?.id)).filter(Boolean);
}
...
function sortColumnsByOrder(columns, columnOrder) {
  const indexedColumns = columns.map((column, index) => ({ column, index }));
  const positionMap = new Map(
    (Array.isArray(columnOrder) ? columnOrder : []).map((id, index) => [
      normalizeId(id),
      index,
    ]),
  );
  ...
  return indexedColumns
    .sort((a, b) => {
      const aPos = positionMap.has(a.column.id)
        ? positionMap.get(a.column.id)
        : Number.MAX_SAFE_INTEGER;
      const bPos = positionMap.has(b.column.id)
        ? positionMap.get(b.column.id)
        : Number.MAX_SAFE_INTEGER;
...
export const useCrmStore = create((set, get) => {
  ...
  const ensurePersistedColumnId = async (columnId) => {
    ...
    const matchingPersistedColumn = columns.find(
      (col) =>
        col.id !== normalizedId &&
        !isTempStageId(col.id) &&
        normalizeColumnTitle(col.title).toLowerCase() ===
          normalizeColumnTitle(localCol.title).toLowerCase(),
    );

    if (matchingPersistedColumn) {
      return { success: true, id: normalizeId(matchingPersistedColumn.id) };
    }

    if (!pendingStageCreations.has(normalizedId)) {
      pendingStageCreations.set(
        normalizedId,
        addColumn(localCol.title, { exceptId: normalizedId }),
      );
    }
  ...
  return {
    columns: sortColumnsByOrder(DEFAULT_STAGES, loadStoredColumnOrder()),
    systemColumns: [],
    archiveColumnId: null,
    leads: [],
    leadOrderByColumn: loadStoredLeadOrderByColumn(),
    isLoading: false,
    error: null,
    hasFetched: false,
    ...
    startPolling: () => {
      const state = get();
      if (state.pollingIntervalId || state.error === "permissions denied")
        return; // Already polling / permission blocked
    ...
    fetchCrmData: async () => {
      ...
      try {
        const [voronkaRes, leadsRes] = await Promise.all([
          apiRequest("/api/v1/sales-manager-crm/voronka", {
            signal: controller.signal,
          }),
          apiRequest("/api/v1/sales-manager-crm/leads", {
            signal: controller.signal,
          }),
        ]);
        ...
        const backendColumns = Array.isArray(voronka)
          ? voronka
          : voronka?.data || [];
        const backendLeads = Array.isArray(leadsRaw)
          ? leadsRaw
          : leadsRaw?.data || [];
        ...
        const orderedColumns = sortColumnsByOrder(
          normalizedColumns,
          loadStoredColumnOrder(),
        );
    ...
    addColumn: async (name, options = {}) => {
      try {
        ...
        if (hasColumnTitle(get().columns, trimmedName, exceptId)) {
          return {
            success: false,
            error: "Bu nomdagi bosqich allaqachon mavjud",
          };
        }
        ...
        if (res.ok) {
          ...
          set((state) => {
            const hasSameColumn = state.columns.some(
              (column) =>
                !isTempStageId(column.id) && column.id === normalized.id,
            );
    ...
    updateColumn: async (id, name) => {
      try {
        if (isTempStageId(id)) {
          return {
            success: false,
            error: "Avval bosqichni serverda yaratish kerak",
          };
        }
        ...
        const res = await apiRequest(
          `/api/v1/sales-manager-crm/voronka/${id}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name: trimmedName }),
        ...
        return {
          success: false,
          error:
            errorData?.message || errorData?.error || "Bosqich tahrirlanmadi",
        };
      } catch (err) {
    ...
    deleteColumn: async (id) => {
      try {
        if (isTempStageId(id)) {
          return {
            success: false,
            error: "Standart bosqichni o'chirib bo'lmaydi",
          };
        }
        ...
        if (hasLeads) {
          return {
            success: false,
            error:
              "Avval ushbu bosqich ichidagi leadlarni boshqa joyga ko'chiring",
          };
        }
        ...
        return {
          success: false,
          error:
            errorData?.message || errorData?.error || "Bosqich o'chirilmadi",
        };
      } catch (err) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          return {
            success: false,
            error:
              errorData?.message || errorData?.error || "Lead qo'shib bo'lmadi",
          };
        }
      } catch (err) {
    ...
    updateLead: async (id, updateData) => {
      try {
        ...
        if (
          ...
          const persistedColumn = await ensurePersistedColumnId(
            updateData.columnId,
          );
          if (!persistedColumn.success) {
    ...
    changeLeadColumn: async (id, columnId) => {
      ...
      const existingLead = get().leads.find(
        (lead) => lead.id === normalizedLeadId,
      );
    ...
    moveLeadByColumnIndex: (id, direction) => {
      ...
      if (
        currentIndex === -1 ||
        targetIndex < 0 ||
        targetIndex >= sameColumnLeads.length
      ) {
        return {
          success: false,
          error:
    ...
    deleteLead: async (id) => {
      try {
        ...
        const res = await apiRequest(
          `/api/v1/sales-manager-crm/leads/${normalizedId}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
        ...
        const errorData = await safeJson(res).catch(() => null);
        const message =
          errorData?.message || errorData?.error || "Lead arxivga yuborilmadi";
        return {
          success: false,
          error: message,
        };
    ...
    moveLeadSameColumn: (activeId, overId) => {
      set((state) => {
        const oldIndex = state.leads.findIndex(
          (lead) => lead.id === normalizeId(activeId),
        );
        const newIndex = state.leads.findIndex(
          (lead) => lead.id === normalizeId(overId),
        );
        if (oldIndex !== -1 && newIndex !== -1) {
    ...
    previewMoveLeadToDifferentColumn: (
      ...
      const activeIndex = leads.findIndex(
        (lead) => lead.id === normalizedActiveId,
      );
      ...
      if (activeIndex !== -1) {
        ...
        const updatedLead = {
          ...activeLead,
          columnId: normalizeId(newColumnId),
        };
        ...
        if (isOverAColumn) {
          ...
          const adjustedOverIndex =
            overIndex >= 0 ? overIndex : newLeads.length;
    ...
    moveLeadToDifferentColumn: async (
      activeId,
      overId,
      newColumnId,
      isOverAColumn,
      ...
      const movedLead = get().leads.find(
        (lead) => lead.id === normalizedActiveId,
      );
      if (!movedLead) {
        set({
      ...
      const result = await updateLead(normalizedActiveId, {
...
useCrmStore.subscribe((state) => {
  writeStorageJson(
    CRM_COLUMN_ORDER_STORAGE_KEY,
    buildColumnOrder(state.columns),
  );
  writeStorageJson(CRM_LEAD_ORDER_STORAGE_KEY, state.leadOrderByColumn || {});
});
